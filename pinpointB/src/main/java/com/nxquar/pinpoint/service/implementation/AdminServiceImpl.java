...
//        @PreAuthorize("hasRole('ROLE_INSTITUTE')")
    ...
    @Override
    public Admin getAdminById(UUID adminId, String jwt) {
    ...
    @Override
    public List<Admin> getAllAdminsByInstitute(UUID instituteId, String jwt) {
        String jwtEmail = jwtService.extractUserName(jwt);
        Institute institute = instituteRepo.findByEmail(jwtEmail);
        if (institute == null || !institute.getId().equals(instituteId)) {
            throw new AccessDeniedException("You are not authorized to access admins for this institute.");
        }

        List<Admin> admins = adminRepo.findByInstituteId(instituteId);
        return admins;
    }
